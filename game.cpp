...
ArmyComposition design_army() {
  ...
  std::cout << "  You have " << budget << " points to build your army."
            << std::endl;
  print_separator();
  ...
  while (budget > 0) {
    ...
    std::cout << "  " << std::left << std::setw(4) << "#" << std::setw(20)
              << "Unit" << std::setw(6) << "Cost" << std::setw(6) << "HP"
              << std::setw(6) << "ATK" << std::setw(6) << "DEF"
              << "Info" << std::endl;
    print_separator();

    for (int i = 0; i < (int)catalog.size(); i++) {
      const auto &u = catalog[i];
      std::cout << "  " << std::left << std::setw(4) << (i + 1) << std::setw(20)
                << u.name << std::setw(6) << u.cost << std::setw(6) << u.hp
                << std::setw(6) << u.atk << std::setw(6) << u.def
                << u.description << std::endl;
    }

    std::cout << std::endl
              << "  Budget remaining: " << budget << " pts" << std::endl;
    ...
    if (max_affordable <= 0) {
      std::cout << "  Can't afford " << selected.name << " (" << selected.cost
                << " pts)." << std::endl;
      continue;
    }
  ...
  print_separator();
  std::cout << "  Army finalized! Remaining budget: " << budget
            << " pts (unspent)" << std::endl;
  print_separator();
...
void display_army(const ArmyComposition &army) {
  ...
  for (auto &kv : army) {
    ...
    std::cout << "  " << std::left << std::setw(20) << t.name << "x"
              << kv.second << "  (HP:" << t.hp << " ATK:" << t.atk
              << " DEF:" << t.def << ")" << std::endl;
    total_cost += t.cost * kv.second;
  }
  std::cout << "  Total spent: " << total_cost << "/" << ARMY_BUDGET << " pts"
            << std::endl;
}
